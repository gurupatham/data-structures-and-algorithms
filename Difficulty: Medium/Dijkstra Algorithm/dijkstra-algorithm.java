...
class Solution {
    ...
    ArrayList<Integer> dijkstra(ArrayList<ArrayList<iPair>> adj, int src) {
        ...
        while(!pq.isEmpty()){
            ...
            for(iPair each : adj.get(pair.first)){
                    if(each.second+dist < ans.get(each.first)){
                        ans.set(each.first,each.second+dist);
                        pq.offer(new iPair(each.first,each.second+dist));
                    }
            }
        }
